  ...
  $.ajax({
    ...
    success: function (object) {
      ...
      $.each(object, function (i, pedido) {
        registro = "No." + pedido.nosolicitud + " Fecha: " + pedido.fecha_registro + " Cte. " + pedido.cliente + " Dpto. " + pedido.departamento + " estado: " + pedido.estado;
        $selectPedidos.append('<option value=' + pedido.id_solicitud + '>' + registro + '</option>');
      });
    }
  });
  ...
  $.ajax({
    ...
    success: function (object) {
      ...
      $.each(object, function (i, producto) {
        registro = "Cdg." + producto.codigo_producto + " Nom. " + producto.nombre_producto + " Cnt. " + producto.cantidad + " Prc. " + producto.precio + " Subtotal: " + producto.subtotal + " Obs." + producto.observaciones;
        $selectProductos.append('<option value=' + producto.codigo_producto + '>' + registro + '</option>');
      });
    }
  });
...
function CargaProductosEdit(idsolicitud) {  
  $.ajax({
    ...
    success: function (object) {      
      ...
      let item = producto.codigo_producto + " - Cnt. " + producto.cantidad + ' - Prc.' + producto.precio + ' - Sbt.' + producto.subtotal + ' - ' + producto.nombre_producto;      
...
function GuardarNuevoRegistro() {
  ...
  $.ajax({
      ...
      // dataType: 'json',
      type: 'post',
      data: { "registro_principal": data1, "detalle_registro": data2,"id_solicitud":solicitudId },
      success: function (object) {
          console.log(object);
      },
